...
// textures
var texP1 = new THREE.FramebufferTexture(OFFSCREEN_SIZE, OFFSCREEN_SIZE, THREE.RGBAFormat);
var texP2 = new THREE.FramebufferTexture(OFFSCREEN_SIZE, OFFSCREEN_SIZE, THREE.RGBAFormat);
 ...
 */
function positionCamera(portalCamera, globalCamera, portal1Mesh, portal2Mesh) {
    // reflect portal camera about portal plane
    var reflected = new THREE.Vector3();
    portal1Mesh.worldToLocal(reflected.copy(globalCamera.position));
...
async function renderRecursive(destCamera, sourceCamera, destPortal, sourcePortal, renderTarget, texture, iteration) {
    if (iteration < recursions) {
        ...
        cameratemp1.position.copy(sourceCamera.position);

        // reposition camera for new render position
        positionCamera(sourceCamera, cameratemp1, destPortal, sourcePortal);
        await renderRecursive(cameratemp1, sourceCamera, destPortal, sourcePortal, renderTarget, texture, iteration);
...
function main() {
    ...
    var animate = async function () {
        ...
        await renderRecursive(camera, cameraP2, portal1, portal2, texPortal1, texP1, 0);
        await renderRecursive(camera, cameraP1, portal2, portal1, texPortal2, texP2, 0);
